...
Our variable analysis gives weight and pushups as numerical variables and position as a categorical variable.  The two variables of interest are weight and position. Graphical displays would include boxplots as a first thing to look at.
#Results
...
```{r}
boxplot(pushups~position,data=players,main="skill vs line weight")
```
...
The question was, "is there a difference in the number of pushups of players in the line and skill positions?"

The null hypothesis $H_0$ is, "there is no difference between the number of pushups of players in the line or skill positions"

The boxplot indicates a significant difference between number of pushups of players in the two positions, the numerical summary shows line players average number of pushups were 17.125, and skill players average number of pushups were 49.72727.

a t-test of difference in mean gave a p-value of .000009421, this is less than 0.05, so we regect the null hypothesis and conclude that we have compelling evidence of a difference in average number of pushups between players in the two positions.